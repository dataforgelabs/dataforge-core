...
DECLARE
    v_ret parameter_map = ROW(null::text, null::text, null::int, null::int, null::int, in_source_id, null::text, null::text, null::jsonb, false)::parameter_map;
...
BEGIN
    ...
    SELECT r.column_alias, r.raw_attribute_id, r.data_type, r.datatype_schema, r.unique_flag
    INTO v_ret.name, v_ret.raw_attribute_id, v_ret.datatype, v_ret.datatype_schema, v_ret.unique_flag
    ...
    SELECT attribute_name, enrichment_id, e.datatype, datatype_schema, e.unique_flag 
    INTO v_ret.name, v_ret.enrichment_id, v_ret.datatype, v_ret.datatype_schema, v_ret.unique_flag
    ...
    SELECT s.name, s.system_attribute_id, s.data_type, (s.name = 's_key' AND s.refresh_type = 'key')
