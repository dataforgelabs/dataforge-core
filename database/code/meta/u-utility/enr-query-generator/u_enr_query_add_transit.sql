...
AND in_parameter.source_relation_ids = e.relation_ids AND e.container_source_id = in_container_source_id;
...
SELECT e.id INTO v_element_id
FROM elements e
WHERE e.type = 'transit' AND e.alias = v_transit_alias AND e.container_source_id = in_container_source_id 
...
-- Attribute from another source: add parent JOIN element
v_parent_element_ids := v_parent_element_ids || meta.u_enr_query_add_join(in_parameter.source_id, in_relation_ids, in_container_source_id); 
v_expression_alias := 'J_' || meta.u_enr_query_relation_alias(in_relation_ids);

v_expression := v_expression_alias || '.' || meta.u_enr_query_get_enrichment_parameter_name(in_parameter);

-- Inserting the transit record
    WITH cte AS (
    INSERT INTO elements ( type, expression, alias, attribute_id, parent_ids, relation_ids, source_id, container_source_id)
    VALUES ( 'transit', v_expression, v_transit_alias, 
    in_parameter.source_relation_parameter_id, v_parent_element_ids, in_relation_ids, in_parameter.source_id, in_container_source_id )
    RETURNING id )
    SELECT id INTO v_ret_element_id
    FROM cte;
...
AND e.alias = v_transit_alias AND e.container_source_id = in_container_source_id 
...
-- Attribute from another source: add parent many-join attribute element
    ...
    v_parent_element_ids := v_parent_element_ids || meta.u_enr_query_add_many_join_attribute(in_parameter,in_container_source_id);
    
    v_expression_alias := 'J_' || meta.u_enr_query_relation_alias(in_parameter.relation_ids);
...
-- Inserting the transit record
    ...
    in_parameter.enrichment_aggregation_id, v_parent_element_ids, in_parameter.relation_ids,v_source_id, in_container_source_id)
    RETURNING id )
    SELECT id INTO v_ret_element_id
    FROM cte;
